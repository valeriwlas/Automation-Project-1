 ...
 */
         ...
         // 29.10.2023
describe('This is first test suite, Valeri Vlassenko', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Valeri')
        cy.get('#lastName').type('Vlassenko')
        cy.get('input[name="password"]').type('Valvla123')
        cy.get('[name="confirm"]').type('Valvla123')
    ...
    */
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('#firstName').type('Valeri')
        cy.get('#lastName').type('Vlassenko')
        cy.get('input[name="password"]').type('Valvla')
        cy.get('[name="confirm"]').type('Valvla')
        cy.get('#username').type('Something')
        cy.get('h2').contains('Password').click()
        cy.get('.submit_button').should('be.disabled')
        cy.get('#success_message').should('not.be.visible')
